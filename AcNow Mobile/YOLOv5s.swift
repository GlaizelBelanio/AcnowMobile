...
class YOLOv5s {
    ...
    init?(useStandaloneModel: Bool) {
        if useStandaloneModel {
            // Load "yolov5s" model and use var_904
            guard let model = try? yolov5s(configuration: MLModelConfiguration()).model else {
                return nil
            }
            self.model = model
            self.outputVarName = "var_904"
            
            print("standalone running")
        } else {
            // Load "best" model and use var_909
            guard let model = try? best(configuration: MLModelConfiguration()).model else {
                return nil
            }
            self.model = model
            self.outputVarName = "var_909"
            
            print("proposed running")
        }
    }
    ...
    func detect(image: UIImage) -> [YOLOv5sOutput] {
        ...
        guard let outputFeatures = try? model.prediction(from: inputFeatures!),
              let outputArray = outputFeatures.featureValue(for: outputVarName)?.multiArrayValue else {
