        ...
        NavigationView {
            VStack {
                ...
                NavigationLink(destination: CaptureView()) {
                    Text("Start")
                           .font(.system(size: 28, weight: .bold)) // font-size: 28px; font-weight: 700;
                           .frame(width: 254, height: 64)
                           .background(Color("buttons")) // Use the buttons color from assets
                           .foregroundColor(.white)
                           .cornerRadius(10)
                           .shadow(radius: 4)
                           .lineLimit(1) // Ensure the text does not wrap
                           .multilineTextAlignment(.center)
                }
                .padding(.top, 30)

                // About Button
                NavigationLink(destination: AboutView()) { // Navigate to AboutView
                    Text("About")
                        .font(.system(size: 28, weight: .bold)) // font-size: 28px; font-weight: 700;
                        .frame(width: 254, height: 64)
                        .background(Color("buttons")) // Use the buttons color from assets
        ...
        VStack {
            ...
            ZStack {
                ...
                if let selectedImage = selectedImage {
                    ...
                    // "X" button to remove the image, positioned at the upper-left corner
                    Button(action: {
                        // Clear the selected image
                        self.selectedImage = nil
                    }) {
                        Text("✖️") // Use an "X" character
                            .font(.system(size: 14))
                            .foregroundColor(.red)
                            .padding(8)
                    ...
                    .padding(.top, -205) // Space from the top
                    .padding(.leading, 275) // Space from the left
                    ...
                    .padding(.top, 290) // Adjust padding to position it correctly
                } else {
...
// When image is selected (for YOLO part)
...
struct ClassifyView: View {
    ...
    var body: some View {
        VStack {
            ...
            // Add any additional UI elements here as needed
        }
        .navigationTitle("Classify") // Set the title for the navigation bar
